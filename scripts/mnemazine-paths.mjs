...
// Canon (per personal constitution structure.md): ~/Brain.
...
export const DEFAULT_VAULT = path.join(os.homedir(), 'Brain')
...
export function resolveVault({ cli, env = process.env.MNEMAZINE_VAULT, requireExists = true } = {}) {
  ...
  if (requireExists && !fsSync.existsSync(vault)) {
    throw new Error(
      ...
      `Set MNEMAZINE_VAULT or pass --vault <path>. Canon default is ~/Brain.`,
    )
  }
  return vault
}
